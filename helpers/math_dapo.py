...
def verify(
    ...
    Args:
        ...
        pause_tokens_index: Indices of pause tokens
        answer_pattern: answer pattern to extract answer from a given tag
...
def compute_score_format(solution_str):
    ...
    try:
        ...
        assistant_blocks = re.findall(r'<\|im_start\|>(?:assistant|tool)\n(.*?)<\|im_end\|>', solution_str, re.DOTALL)
        # breakpoint()
        format_reward = 0.0
        ...
        for i, assistant_block in enumerate(assistant_blocks[:-1]):
            if assistant_block.count('<think>') == 1 and assistant_block.count('</think>') == 1 and assistant_block.count('<tool_call>') == 1 and assistant_block.count('</tool_call>') == 1:
                ...
                # print(f"assistant_block: {assistant_block}")
                # breakpoint()
                if think_match:
...
def compute_score(
    ...
    strict_box_verify: bool = False,
    pause_tokens_index: Optional[list[int]] = None,
    answer_pattern: str = r"(?i)<answer>(.*?)</answer>",
) -> float:
    ...
    # acc = correct
    # if format_score == 1.0:
    #     reward = reward + 0.5*format_score
    #     if np.random.randint(0,500) == 1:
    #         print(f'pred: {tmp}')
    # elif format_score == 0.0:
    #     reward = 0.5 * reward + format_score
    # else:
    #     reward = format_score + 0.1 * reward
...
def math_dapo_reward(completion: str, ground_truth: dict):
    """Return a reward dict with keys 'score' and any debug info."""
    # extract the ground truth for this example
...
def load_math(parquet_path: str) -> Dataset:
    """
    Load the dapo-math Parquet into a 🤗 Dataset with:
      - question: the math prompt + formatting instruction
      - ground_truth: the string answer (from the reward_model column)
    Handles numpy.ndarray prompts and stringified dicts in reward_model.
    """
    ...
    for _, row in df.iterrows():
        ...
        if isinstance(raw, np.ndarray):
            ...
            raise ValueError(f"Unexpected prompt type: {type(raw)}")
        ...
        raw_meta = row["reward_model"]
        if isinstance(raw_meta, str):
            meta = ast.literal_eval(raw_meta)
        elif isinstance(raw_meta, dict):
            meta = raw_meta
        else:
            raise ValueError(f"Unexpected reward_model type: {type(raw_meta)}")

        gt = meta.get("ground_truth")
        if gt is None:
            raise KeyError(f"No 'ground_truth' in reward_model: {meta}")

        records.append({
            "question": q,
            "ground_truth": str(gt),
            "support_material_path": []
        })
